...
Scripts (`scripts/`) : `horn-init.ps1`, `horn-check.ps1`, `horn-fingerprint.ps1`, `horn-doctor.ps1`, `horn-package.ps1`, tous avec `-ProjectDir <dossier>` obligatoire. Références (`references/`) : grille de vérifications par domaine, workflow, schéma de configuration, correspondance Superpowers et Context7, anciens noms. Modèles (`templates/`).
