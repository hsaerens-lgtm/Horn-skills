...
- Première exécution de `quality.yml` : job « Tests du dispositif (Windows PowerShell 5.1) » **RÉUSSI** ; job « Recherche de secrets (gitleaks-action@v3) » **ÉCHOUÉ** avec « Unexpected exit code [1] » (erreur d'exécution, pas une fuite : une fuite donnerait le code 2). Cause confirmée dans les journaux fournis par l'utilisateur : l'action exécute `gitleaks detect --log-opts="--first-parent 1ea1127^..8d01bc4"`, et `1ea1127` est le commit racine, sans parent : Git répond « unknown revision », gitleaks sort en code 1 (scan partiel, 0 octet). Défaut connu de `gitleaks-action` au tout premier push d'un dépôt ; l'historique complet avait été scanné localement (aucun secret). Seconde exécution (commit `20bb978`, avec un parent) : **SUCCÈS**, les deux jobs (tests Windows, Gitleaks) RÉUSSIS. La CI est donc opérationnelle.
