...
| Conception d'interface (`design`, 0.4.0) | `/horn-dev:design …` ou via `dev` (intention « concevoir une interface ») | oui | à confirmer dans une nouvelle session | par projet (système de design existant prioritaire) | format validé ; comportement non testé | React Bits n'est utilisé que si le skill `anthropic-skills:reactbits` est présent dans la session (fourni par l'application Claude, pas par horn-dev) ; sinon installation officielle par composant, signalée |
...
| React Bits (`anthropic-skills:reactbits`) | skill fourni par l'application Claude (pas installé par horn-dev), 135 composants React TS + Tailwind, 4 Mo | oui dans cette session (desktop) ; non garanti dans la CLI terminal | catalogue et procédure d'installation lus le 2026-09-10 | composants animés pour `/horn-dev:design` ; repli : `npx shadcn@latest add https://reactbits.dev/r/<Composant>-TS-TW.json` |
