...
6. horn-dev: `claude plugin marketplace add "<absolute path of this folder>"` then `claude plugin install horn-dev@horn-toolbox --scope user`. Verify with `claude plugin list` (horn-dev, Scope: user) and `claude plugin details horn-dev@horn-toolbox` (10 skills: bugfix, check, design, dev, feature, init, release, resume, review, testing).
